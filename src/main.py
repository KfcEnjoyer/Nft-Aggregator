...
def get_address():
    ...
    if database.check_if_exists(nft_address):
        ...
        return render_template("index.html", mint=value[0][0], name=value[0][1], standard=value[0][2], owners=value[0][3])
    else:
        database.insert_value(response['mint'], json.dumps(response))
        value = database.get_value(nft_address)
        return render_template("index.html", mint=value[0][0], name=value[0][1], standard=value[0][2], owners=value[0][3])
